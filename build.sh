...
if command -v /usr/local/texlive/2015basic/bin/universal-darwin/xelatex >/dev/null 2>&1; then
  mkdir -p output;
  for f in *-resume.tex; do
    /usr/local/texlive/2015basic/bin/universal-darwin/xelatex -output-dir=output $f;
