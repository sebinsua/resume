...
\inlineheadsection  % special section that has an inline header with a 'hanging' paragraph
  ...
  {Full-stack software design and implementation. I follow the mantra \emph{the right tool for the right job} and
...
\headedsection  % sets the header for the section and includes any subsections
  {\href{http://hire.duplo.tech}{Duplo Tech}}
  ...
  \headedsubsection
    ...
    \begin{itemize}[leftmargin=0cm]
      ...
      \begin{enumerate}[leftmargin=0.5cm]
          \item I auto-generated tests from pre-existing code.
          \item I made open-source contributions to the libraries `cypress` and `cypress-image-snapshot`.
          \item I reduced the time the tests took to run from ~25 minutes to ~10 minutes by writing a codemod that auotomated rewriting 10,000s of lines of code to use code-splitting techniques.
          \item I wrote cross-platform code and dockerized Cypress so that it could be used by engineers on Mac, Windows and Linux machines (otherwise image regressions break due to OS, browser, and font rendering differences).
          \item I wrote an Argo workflow that parallelised the tests on a Kubernetes cluster, and that effectively allowed test runs that would otherwise take hours, to run in under a minute.
      \end{enumerate}
      \item Introduced TypeScript into a large React project.
      \item Upgraded a large 50+ component library from v3 of Material UI to v4.
    \end{itemize}
    }}
}
...
\headedsection
  ...
  \begin{itemize}[leftmargin=0cm]
      \item Architect on the HTC Watch project in which I later lead a team of five developers. Due to my early work on internationalisation and implementation of multiple payment services we were able to scale this up to ~20 countries.
      \item Significant input in re-engineering development process as we moved from being a startup to a larger company. For example: an engineering culture; Git instead of SVN; continuous integration; modern deployment tools; etc.
  ...
  \begin{itemize}[leftmargin=0cm]
      ...
      \item A web service for Paramount Studio
      s the League of Extraordinary Dancers iPhone app. This was interoperable with the iTunes video store in order to check receipts.
  \end{itemize}
    }}
}
